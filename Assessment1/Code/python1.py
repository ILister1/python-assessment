...
def two(input):
	...
	if upper_case.endswith("BERT") and upper_case.startswith("BERT"):
		lower_case = upper_case[4:len(upper_case) - 4]
		return lower_case.lower()
	else:
		return ""
...
def six(input):
	if "cei" in input or "ie." in input:
		return True
	if "cie" in input or "ei." in input:
		return False
	else:
		return True
...
def eight(input):
	number_fact = math.factorial(input)
	return number_fact
...
def nine(inputString, char):
	...
	if char not in newstring:
		return -1
	elif char in newstring:
		char_pos = newstring.find(char)
		char_real = char_pos + 1
	return char_real
